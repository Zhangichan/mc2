...
//重写右键菜单
void PagesTextEdit::contextMenuEvent(QContextMenuEvent *event)
{
    QMenu *menu = new QMenu;

    //QKeySequence类是专门封装快捷键的，这里使用的是默认的快捷键操作，其快捷键位"&"号后面那个字母
    QAction *cut = menu->addAction(tr("剪切"), this, SLOT(cut()), QKeySequence::Cut);
    cut->setIcon(QIcon(":/images/edit-cut.svg"));
    cut->setEnabled(textCursor().hasSelection());

    QAction *copy = menu->addAction(tr("复制"), this, SLOT(copy()), QKeySequence::Copy);
    copy->setIcon(QIcon(":/images/edit-copy.svg"));
    copy->setEnabled(textCursor().hasSelection());

    QAction *paste = menu -> addAction(tr("粘贴"), this, SLOT(paste()), QKeySequence::Paste);
    paste->setIcon(QIcon(":/images/edit-paste"));
    ...
    menu->addAction(menucolor);
    menu->addAction(menubold);
    menu->addAction(menuitalic);
    menu->addAction(menuunderline);
    menu->addAction(menustrickout);
    menu->addAction(menuonline);

    menu->addSeparator();//增加分隔符

    menu->addAction(menupicture);
    menu->addAction(menutable);
    QMenu *pmenu = new QMenu(tr("段落"));
    pmenu->addAction(menuleft);
    menuleft->setEnabled(!document()->isEmpty());
    pmenu->addAction(menucenter);
    menucenter->setEnabled(!document()->isEmpty());
    pmenu->addAction(menuright);
    menuright->setEnabled(!document()->isEmpty());
    pmenu->addAction(menufull);
    menufull->setEnabled(!document()->isEmpty());
    pmenu->addSeparator();
    pmenu->addAction(menuindentM);
    menuindentM->setEnabled(!document()->isEmpty());
    pmenu->addAction(menuindentL);
    menuindentL->setEnabled(!document()->isEmpty());
